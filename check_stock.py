...
def check_tcgcompany_category(store: dict, state: dict) -> tuple[list[dict], bool]:
    """Check een TCGCompany categoriepagina via Playwright (geblokkeerd door Cloudflare)."""
    print("  [JS] TCGCompany — Playwright rendering...")
    base_url = store["url"].rstrip("/")

    html = fetch_page_js(base_url, wait_selector="div.inner-loop-product-holder")
    if not html:
        return [], False
    ...
    for page in range(2, max_page + 1):
        page_html = fetch_page_js(
            f"{base_url}/page/{page}/",
            wait_selector="div.inner-loop-product-holder",
        )
        if not page_html:
            break
        all_products.extend(parse_woocommerce_loop_cards(BeautifulSoup(page_html, "lxml")))

    if not all_products:
        print("  [!] Geen TCGCompany productkaarten gevonden na JS-rendering")
        return [], False
...
def check_category(store: dict, state: dict) -> tuple[list[dict], bool]:
    ...
    # Magento vóór WooCommerce: Magento gebruikt li.product-item, WooCommerce li.product
    # Sommige Magento-themes voegen ook class 'product' toe aan li.product-item,
    # waardoor de WC-selector vals positief triggert als Magento als eerste stond.
